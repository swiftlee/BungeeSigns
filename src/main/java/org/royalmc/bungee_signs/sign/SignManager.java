...
/**
 * This is the class for managing the loading, creation and removal of {@link StatusSign} as well as there updating via a Ping to the requested server.
 * 
 * @author GregZ_
 * @since 1.1
 *
...
public class SignManager {
			...
			public void run() {
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
					...
					public void run() {
						...
						if (serverIsOffline == false) {
							if (plugin.getConfig().getBoolean("isUHCServer") == false) {
								syncUpdate(sign, status, onlinePlayers, maxPlayers);
							} else {
								if (BungeeSigns.signInProgress.get(sign) == null)
									BungeeSigns.signInProgress.put(sign, false);

								if (onlinePlayers > 0) {
									if (BungeeSigns.signInProgress.get(sign) == false) {
	...
	@SuppressWarnings("deprecation")
	public void updateWool(Sign s, byte value) {
		...
		Location left = new Location(world, x - 1, y, z);
		Location right = new Location(world, x + 1, y, z);
		Location up = new Location(world, x, y + 1, z);
		Location down = new Location(world, x, y - 1, z);
		Location front = new Location(world, x, y, z + 1);
		Location back = new Location(world, x, y, z - 1);
